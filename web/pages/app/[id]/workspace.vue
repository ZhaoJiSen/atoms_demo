...
import {
  Home, RefreshCw, Sparkles, Loader2, AlertTriangle, Plus,
  FileText, LayoutGrid, Zap, Database, Folder, Eye,
  Send, Bot, User, CheckCircle, XCircle, Circle, Check, MessageSquare
} from '@lucide/vue'
...
const tabItems = [
  { key: 'preview', labelKey: 'workspace.tabs.preview', iconName: 'Eye' },
  { key: 'files', labelKey: 'workspace.tabs.files', iconName: 'Folder' },
  { key: 'pages', labelKey: 'workspace.tabs.pages', iconName: 'LayoutGrid' },
  { key: 'api', labelKey: 'workspace.tabs.api', iconName: 'Zap' },
  { key: 'models', labelKey: 'workspace.tabs.models', iconName: 'Database' },
  { key: 'spec', labelKey: 'workspace.tabs.spec', iconName: 'FileText' },
  { key: 'chat', labelKey: 'workspace.tabs.chat', iconName: 'MessageSquare' },
]
...
async function loadApp() {
  ...
  try {
    ...
    if (app.value.status === 'pending' && isAuthenticated.value) {
  ...
  <div class="flex-1 flex flex-col">
    ...
    <template v-else>
      ...
      <div class="flex-1 overflow-y-auto p-6">
        <template v-if="activeTab === 'preview'">
          <div v-if="app.result?.preview">
            <PreviewMockApp :preview="app.result.preview" :app-title="app.title" />
          </div>
          <div v-else class="text-center py-12">
            <Eye class="w-12 h-12 text-zinc-600 mx-auto mb-3" />
            <p class="text-sm text-zinc-500">{{ t('workspace.previewNotGenerated') }}</p>
          </div>
        </template>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showGeneratingModal" class="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div class="relative w-full max-w-md mx-4 bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div class="px-6 py-4 bg-zinc-950/50 border-t border-zinc-800">
